...
In general a Diffusion Model consists of three parts: the forward process, the denoising network and the sampling procedure.

In the forward process the image will get more and more random during the steps of adding gaussian noise with the following formula: $x_t = \sqrt{1-\bar{\alpha}_t}x_0 + \sqrt{\bar{\alpha}_t}ϵ_t$, where $\bar{\alpha}_t = \Pi^{t} \alpha_i$
